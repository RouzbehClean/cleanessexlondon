...
export function validate(data: ParsedWorkbook): ValidationError[] {
  ...
  // Note: duplicate primary key rows are silently de-duplicated at insert time
  // (first occurrence wins) — they are not flagged as validation errors here,
  // since spreadsheets often repeat a site/cleaner across multiple worksheet rows.
